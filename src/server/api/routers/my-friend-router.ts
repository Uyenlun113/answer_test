  ...
  getById: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return ctx.db.connection().execute(async (conn) => {
         ...
         */
        const mutualFriendCountResult = await conn
          .selectFrom('friendships as fa')
          .innerJoin('friendships as fb', 'fb.friendUserId', 'fa.friendUserId')
          .where('fa.status', '=', FriendshipStatusSchema.Values['accepted'])
          .where('fa.userId', '=', ctx.session.userId)
          .where('fb.userId', '=', input.friendUserId)
          .select((eb) =>
            eb.fn.count('fa.friendUserId').as('mutualFriendCount')
          )
          .groupBy('fa.userId')
          .executeTakeFirst()
